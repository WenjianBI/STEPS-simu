...
## Reproduce results in table-0.05.csv (the same as Figures 1-2) based on out.steps generated by STEPS.simu.example()
## For more parameter settings, please refer to https://github.com/WenjianBI/STEPS-simu for more parameter settings

toe_SV=sum(out.steps[,"p-value"]<0.05,na.rm = T)/sum(!is.na(out.steps[,"p-value"]))
para_SV=mean(out.steps[,"par.est"],na.rm = T)
sd_SV=sd(out.steps[,"par.est"],na.rm = T)
mean.se_SV=mean(sqrt(out.steps[,"var.est"]),na.rm = T)
